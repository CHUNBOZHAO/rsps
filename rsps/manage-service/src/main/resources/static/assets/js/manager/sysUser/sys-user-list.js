$(function () {
    ...
    // 加载数据
    $.ajax({
        url:"/manager/sysUser/list/content",
        dataType:"html",
    ...
    $(".sys-user-mode").click(function () {
        window.location.href = "/manager/sysUser/topology/index";
    });

    // 新增
    $(document).on("click", ".sys-user-add", function () {
        window.location.href = "/manager/sysUser/edit/show";
    });

    // 编辑
    $(document).on("click", ".btn-edit", function () {
        var userId = $(this).attr("userId");
        window.location.href = "/manager/sysUser/edit/show?userId=" + userId;
    });
        ...
        $.confirm({
            ...
            buttons: {
                tryAgain: {
                    ...
                    action: function(){
                        $.ajax({
                            url:"/manager/sysUser/delete/handle/" + userId,
                            success:function (msg) {
                                if (msg == "success") {
                                    toastr.success("删除成功");
                                    setTimeout(function () {
