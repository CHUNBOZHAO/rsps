...
public class ManagerReportController {
    ...
    public String showShipmentBoxOperateRecordInfoContent(ModelMap modelMap, HttpServletRequest request) {
        ...
        String userId = request.getParameter("forwarder");

        List<OperatorInfoVO> operatorInfoVOS = null;
        List<LineInfoVO> lineInfos = null;
        ...
        if(userId==null){
            operatorInfoVOS   = operatorInfoService.getOperatorsByUserId(userDetail.getEntCode().concat("_"), userDetail.getUserId());
            lineInfos = lineService.queryChildLines(userDetail.getUserId(), userDetail.getEntCode().concat("_"));

            // 获取承运商
            modelMap.put("isSelected1","1");
            sysUserInfos = sysUserService.getSiteUserInfoByUserId(userDetail.getUserId());

        }else{
            operatorInfoVOS   = operatorInfoService.getOperatorsByUserId(userDetail.getEntCode().concat("_"), userId);
            lineInfos = lineService.queryChildLines(userId, userDetail.getEntCode().concat("_"));

            // 获取承运商
            modelMap.put("isSelected", userId);
            sysUserInfos = sysUserService.getSiteUserInfoByUserId(userDetail.getUserId());
