...
public class OperatorInfoServiceImpl extends AbstractCrudService<OperatorInfoDO> implements OperatorInfoService {
     ...
     */
    @Override
    public boolean checkLogged(String userName, String operatorId, String imei, String entCode) {
        boolean res = false;
        // 判断本次登录的imei是否一致
        Integer resInt = operatorInfoDao.checkImei(userName, operatorId, imei, entCode.concat("_"));
        if (resInt <= 0) { // imei不一致
            // 判断位置上传时间
            res = operatorLocationService.checkOnline(userName, operatorId, 180);
        }
        return res;
    }
    ...
    public List<OperatorInfoVO> getOperatorsByUserId(String entCode, String userId) {
        ...
        try {
            ...
            for (OperatorInfo operatorInfo : operatorInfos) {
                ...
                operator.setPlateNumber(operatorInfo.getPlateNumber());
    ...
    public boolean saveInfo(OperatorInfoVO operatorInfoVO, String entCode) {
        ...
        try {
            ...
            info.setPlateNumber(operatorInfoVO.getPlateNumber());
    ...
    public OperatorInfoVO getInfoById(Long id, String entCode) {
        ...
        try {
            ...
            if (operatorInfo != null) {
                ...
                infoVO.setPlateNumber(operatorInfo.getPlateNumber());
            }
...
//            update(infoDO);
    ...
    public boolean checkOperatorNo(String operatorNo, String entCode,Integer id) {
        boolean flag = false;
        try{
            Integer count = operatorInfoDao.checkOperatorNo(operatorNo, entCode,id);
            if(count==1) {
                flag = true;
            }
        }catch (Exception e){
