$(function () {
    ...
    var curTransferId = $("#curTransferId").val();
    // 加载数据
    $.ajax({
        url:"/manager/line/related/transfer/list",
        data:"lineId=" + lineId + "&curTransferId=" + curTransferId,
        dataType:"html",
    ...
    function relateAction(data, dataArray, myModal) {
        $.ajax({
            url:"/manager/line//transfer/relate/do",
            data:data,
            dataType:"json",
    ...
    function relateCancelAction(data, dataArray, myModal) {
        $.ajax({
            url:"/manager/line/transfer/relate/cancel",
            data:data,
            dataType:"json",
            success:function (msg) {
                setTimeout(function () {
    ...
    $(document).on("click", "#btn_batch_relate", function () {
        ...
        if (isChecked) {
            ...
            var params = $(".item-unrelated:checked").map(function () {
                var res = "transferIds=" + $(this).val();
                dataArray.push($(this).val());
                return res;
            }).get().join("&");
            if (dataArray.length > 80) {
                toastr.warning("一次最多关联80项，请重新选择");
            } else {
    ...
    $(document).on("click", "#btn_batch_cancel_relate", function () {
        ...
        if (isChecked) {
            ...
            if (dataArray.length > 80) {
                toastr.warning("一次最多取消关联80项，请重新选择");
            } else {
                var lineId = $(this).attr("lineId");
                var data = "lineId=" + lineId + "&" + params;
                var myModal = $("#process_modal").modal({
    ...
    $(document).on("click", ".btn-relate", function () {
        var lineId = $(this).attr("lineId");
        var userId = $(this).attr("transferId");
        var data = "lineId=" + lineId + "&transferIds=" + userId;
        var dataArray = new Array();
