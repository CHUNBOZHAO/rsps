$(function () {
    ...
    // 加载数据
    $.ajax({
        url:"/manager/line/related/line/list",
    ...
    function relateAction(data, dataArray, myModal) {
        $.ajax({
            url:"/manager/line/line/relate/do",
            data:data,
            dataType:"json",
    ...
    function relateCancelAction(data, dataArray, myModal) {
        $.ajax({
            url:"/manager/line/line/relate/cancel",
            data:data,
            dataType:"json",
            success:function (msg) {
                setTimeout(function () {
    ...
    $(document).on("click", "#btn_batch_relate", function () {
        ...
        if (isChecked) {
            ...
            var params = $(".item-unrelated:checked").map(function () {
                var res = "lineIds=" + $(this).val();
                dataArray.push($(this).val());
                return res;
            }).get().join("&");
            if (dataArray.length > 100) {
                toastr.warning("一次最多关联100项，请重新选择");
            } else {
                var customId = $(this).attr("customId");
                var data = "customId=" + customId + "&" + params;
    ...
    $(document).on("click", "#btn_batch_cancel_relate", function () {
        ...
        if (isChecked) {
            ...
            if (dataArray.length > 100) {
                toastr.warning("一次最多取消关联100项，请重新选择");
            } else {
                var customId = $(this).attr("customId");
                var data = "customId=" + customId + "&" + params;
                var myModal = $("#process_modal").modal({
    ...
    $(document).on("click", ".btn-relate", function () {
        ...
        var customId = $(this).attr("customId");
        var data = "lineIds=" + lineId + "&customId=" + customId;
        var dataArray = new Array();
        dataArray.push(lineId);
        var myModal = $("#process_modal").modal({
            backdrop:'static',
            keyboard:false
        });
